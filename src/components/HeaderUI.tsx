...
const HeaderUI = ({isDark, toggleTheme} : {isDark: boolean, toggleTheme: () => void}) => {
        ...
        <header className='fixed top-0 left-0 right-0 z-50 backdrop-blur-lg border-b bg-white/80 dark:bg-neutral-950/80'>
            <div className='flex justify-between items-center mx-auto px-4 py-4 text-slate-700 dark:text-neutral-100 max-w-7xl'>
                ...
                <nav className='hidden lg:flex items-center space-x-8'>
                    {navItems.map((item) => (
                        <Button className='relative group opacity-80 hover:opacity-100 transition-colors duration-300 dark:text-slate-200' key={item.href} onClick={() => handleScrollToSection(item.href)}>
                            {item.label}
            ...
            {isMobileMenuOpen && (
                <nav className='lg:hidden p-4 border-t border-border/50 animate-fade-in flex flex-col gap-1'>
                    {navItems.map((item) => (
                        <Button className='text-left opacity-80 rounded-sm hover:opacity-100 hover:bg-slate-900/20 dark:hover:bg-slate-200/20 py-2 dark:text-slate-200 px-4 transition-all duration-300' key={item.href} onClick={() => handleScrollToSection(item.href)}>
                            {item.label}
                        </Button>
                    ))}
                </nav>
